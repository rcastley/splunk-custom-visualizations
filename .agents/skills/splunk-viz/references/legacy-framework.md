...
## Legacy Step 1: Gather Requirements
...
## Legacy Step 2: Generate the App
...
When creating a new viz, read `core-template.md` for the complete AMD module template. It includes all lifecycle methods (`initialize`, `getInitialDataParams`, `formatData`, `updateView`, `reflow`, `destroy`), the `_lastGoodData` caching pattern, `_status` sentinel handling, HiDPI canvas setup, and the `_drawStatusMessage` / `_ensureCanvas` helpers. Use it as the starting point — do not deviate from the structure.
...
25. **Drilldown from Canvas-based visualizations**. When the viz needs click-through to drill into data, read `drilldown.md` for the full hit-rect tracking pattern, click/hover handlers, Dashboard Studio drilldown configuration, and token format. Always document drilldown setup in the viz README.
...
Before delegation or unattended execution, verify that the active agent environment permits Python, dependency installation, npm build commands, and `./build.sh`. Permission configuration is host-specific and must remain outside this portable skill.
...
## Legacy Step 3: Generate Build Script
...
For drawing helpers (color scales, rounded rects, arcs, legends, grids, responsive text, hit testing), read `canvas-recipes.md`.

For smooth real-time animation (client-side tweening between SPL samples), read `smoothing.md`. This covers both single-value tweens (gauges, needles) and per-entity position tweens (moving items on a map), with the full timer/cleanup implementation.

## Splunk Design Guidelines Reference

For chart-style vizs that use axes, legends, gridlines, or tooltips, read `design-guidelines.md` for official Splunk font stacks, color palettes (semantic, categorical, sequential, divergent), spacing constants, description best practices, and responsive design guidance. Canvas-only vizs (gauges, status boards) can skip this — use `sans-serif` and `monospace` per rule 10.
...
## Legacy Step 4: Verify Completeness
...
## Legacy Step 5: Generate Test Harness Config (MANDATORY)
...
## Legacy Step 6: Scaffold a Parent Dashboard App (Optional)

When the user asks to scaffold a parent Dashboard Studio app that bundles **legacy** custom visualizations, read `dashboard-studio-app.md` for the full directory structure, file templates, build pipeline, and namespace guidance. This reference does not describe the native Dashboard Studio extension framework.
